...
export default function LinaAssistant({ onClose }: LinaAssistantProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: 'Привет! Я Лина, ваш ИИ-помощник. Готова помочь с созданием сайтов, настройкой проектов или любыми вопросами по poehali.dev!',
      isUser: false,
      timestamp: new Date()
    }
  ]);
  ...
  const getSmartResponse = (input: string): string => {
    ...
    // Вопросы о poehali.dev
    if (lowerInput.includes('poehali') || lowerInput.includes('поехали')) {
      return 'poehali.dev — это сервис для создания сайтов через русский язык. Здесь можно создавать React-приложения, публиковать их в интернете и синхронизировать с GitHub. Всё делается прямо в браузере без установки программ!';
    }
    
    // Вопросы о создании сайтов
    if (lowerInput.includes('сайт') || lowerInput.includes('создать') || lowerInput.includes('приложение')) {
      return 'Для создания сайта просто опишите, что хотите получить. Например:\n• "Создай лендинг для кафе с меню"\n• "Сделай портфолио дизайнера"\n• "Нужен интернет-магазин одежды"\n\nЯ создам всё за несколько минут!';
    }
    
    // Вопросы о публикации
    if (lowerInput.includes('опубликовать') || lowerInput.includes('домен') || lowerInput.includes('хостинг')) {
      return 'Чтобы опубликовать сайт:\n1. Нажмите "Опубликовать" в редакторе\n2. Выберите бесплатный поддомен или подключите свой\n3. SSL-сертификат выпустится автоматически\n\nСайт станет доступен в интернете за пару минут!';
    }
        ...
        <div className="p-6 border-t bg-gray-50">
          <div className="flex gap-3">
            <textarea
              ...
              placeholder="Задайте любой вопрос о создании сайтов..."
              rows={2}
          ...
          <p className="text-xs text-gray-500 mt-2 text-center">
            Лина готова помочь с любыми вопросами по poehali.dev! 🚀
          </p>
        </div>
      </div>
    </div>
